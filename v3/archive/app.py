...
from owcs_day_script_writer_v2 import generate_day_script_v2
...
if mode == "DAY 대본 생성":
    ...
    opening_images = st.file_uploader(
        "오프닝 참고 이미지",
        type=["png", "jpg", "jpeg", "webp"],
        accept_multiple_files=True,
    )

    opening_memo = st.text_area(
        "오늘의 화제 / 작가 메모",
        height=120,
        placeholder="예: O2 Blast 2연승, T1 vs ZETA 명승부, Stage2 2주차 시작",
    )
    ...
    if st.button("전체 DAY 대본 생성 V2", type="primary"):
        opening_image_names = [img.name for img in opening_images] if opening_images else []

        script = generate_day_script_v2(
            language=language,
            event_name=event_name,
            day_label=day_label,
            date_text=date_text,
            ...
            opening_image_names=opening_image_names,
        )
    ...
    if "day_script" in st.session_state:
        ...
        st.download_button(
            ...
            file_name=f"{event_name}_{day_label}_{language}_script_v2.txt",
            mime="text/plain",
        )
